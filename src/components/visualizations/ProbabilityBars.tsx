          ...
          >
            <div className="flex items-center justify-between text-[12px] font-mono">
              ...
              <div className="flex items-center gap-2.5">
                <span className={`text-[11px] w-5 text-right font-bold tabular-nums ${isWinner ? 'text-emerald-400' : 'text-text-muted'}`}>
                  #{cand.rank}
                </span>
                ...
                {isWinner && (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-300 uppercase tracking-wider bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30">
              ...
              <div className="flex items-center gap-4">
                {showLogit && (
                  <span className="text-[11px] text-text-muted hidden sm:inline font-mono">
                    logit: <span className="text-white font-medium tabular-nums">{cand.logit.toFixed(2)}</span>
                  </span>
                )}
            ...
            >
              <motion.div
                ...
                className={`h-full rounded-full ${
                  isWinner
                    ? 'bg-gradient-to-r from-emerald-400 to-emerald-300 shadow-[0_0_12px_rgba(16,185,129,0.9)]'
                    : idx === 0
                    ? 'bg-emerald-500'
                    : 'bg-emerald-500/40'
                }`}
