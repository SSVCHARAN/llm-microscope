    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-emerald-500/30 bg-black/60 p-6 shadow-2xl">
        ...
        <p className="text-[12px] leading-relaxed text-[#C0C0C0]">
          The attention heatmap percentages above are used to multiply the <strong>Value vectors (V)</strong> from Stage 3. For the last token <code className="text-white">"{attentionOutput.tokenText}"</code> (which is responsible for predicting the next word), the model gathers context from previous tokens based on row 4 of the heatmap:
        </p>
        ...
        <div className="flex flex-wrap gap-2 text-[11px] font-mono">
          {attentionOutput.breakdown.map((item) => (
            ...
            >
              ...
              <span className="text-white">of V("{item.sourceToken}")</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-start gap-3 p-3.5 rounded-lg bg-emerald-500/[0.08] border border-emerald-500/20 text-[12px] text-emerald-200">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="leading-relaxed">
              In <strong>Stage 5 (Feed-Forward Network)</strong>, this Attention Context Vector <code className="text-emerald-300">z</code> is added to the token's original vector via Residual Connection (<code className="text-white">x_orig + z</code>) to produce the exact 4 values <code className="text-white">[+0.72, -0.45, +1.14, -0.88]</code>. Those 4 values become the <strong>Input Nodes (x₁, x₂, x₃, x₄)</strong> in the Stage 5 neural graph!
            </p>
          </div>
        </div>
      </div>
